...
podTemplate(label: 'testing', 
  containers: [
    containerTemplate(name: 'kubectl', image: 'baoannguyen/kubectl:1.7.0', ttyEnabled: true, command: 'cat'),
  ...
  node ('testing') {
    ...
    dir('app') {
      stage('Checkout the Notepad application') {
        git url: 'https://bitbucket.org/an_npb/note_app.git', branch: "${GIT_BRANCH}", credentialsId: 'bitbucket-app-password'
      }
...
pipeline {
    ...
    stages {
        stage("build") {
            steps {
                   print "DEBUG: parameter foo = ${params.Nodes}"
            }
        }
    }
}